...
-- Catch any FK added by a later migration without relying on a hand-maintained list.
do $$
declare
  r record;
  idx_name text;
begin
  ...
  loop
    ...
    execute format('create index if not exists %I on %s (%s)', idx_name, r.table_name, r.columns);
  end loop;
end $$;
